/**
 * @file main.cpp
 * Main file for the SRT to SAMI Converter. Processes command line options.
 *
 * @version 1.2.0
 * @author Ivan Milovanov
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    TCLAP::CmdLine cmd(
        ...
        "If called without any arguments, all .srt files in the current "
        "directory "
    ...
    TCLAP::UnlabeledValueArg<string> inputArg(
        ...
        "(to convert files in a directory and it's subdirectories, include the "
        "-r switch).",
        false, ".", "string");
    cmd.add(inputArg);
    ...
    TCLAP::SwitchArg recursiveArg(
        "r", "recursive",
        "If the input is a directory, this flag indicates its subdirectories "
        "will be searched "
        "recursively for .srt files to convert. This flag is ignored if the "
        "input path is a file.");
    cmd.add(recursiveArg);

    TCLAP::SwitchArg verboseArg("v", "verbose",
                                "Indicates that details about the conversion "
                                "should be printed to the console.");
    cmd.add(verboseArg);

    TCLAP::SwitchArg quietArg(
        "q", "quiet",
        "Prevents details about the conversion from being printed to the "
        "console. Overrides --verbose.");
    cmd.add(quietArg);
