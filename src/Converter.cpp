...
Converter::Converter(int timeOffsetMs, const string& outputDir, bool quiet,
  ...
  if (!_outputDir.empty() &&
      !Utils::isDir(_outputDir)) {  // Create the output directory
    if (_verbose) {
...
int Converter::convertFile(string filepath) {
  ...
  string outpath =
      regex_replace(filepath, regex("\\.srt$", regex_constants::icase),
                    string("")) +
      ".vtt";

  if (!_outputDir.empty()) {  // The user specified an output directory
    outpath = _outputDir + DIR_SEPARATOR +
              outpath.substr(outpath.find_last_of(DIR_SEPARATOR) + 1);
  }
  ...
  try {
    ...
    wregex rgxTimeFrame(
        L"(\\d\\d:\\d\\d:\\d\\d,\\d{3}) --> (\\d\\d:\\d\\d:\\d\\d,\\d{3})");
    ...
    for (;;) {
      ...
      if (regex_match(sLine, rgxDialogNumber)) continue;
      ...
      if (!matchTimeFrame.empty()) {
        if (_timeOffsetMs != 0) {
          ...
          sLine = msToSmiTimeString(msStartTime) + L" --> " +
                  msToSmiTimeString(msEndTime);
        } else {
    ...
    if (_quiet)
      cerr << "ERROR converting file \"" << filepath << "\": " << e.what()
           << endl;
    else
...
wstring Converter::msToSmiTimeString(int ms) {
  ...
  return (hours < 10 ? L"0" : L"") + to_wstring(hours) + L":" +
         (minutes < 10 ? L"0" : L"") + to_wstring(minutes) + L":" +
         (seconds < 10 ? L"0" : L"") + to_wstring(seconds) + L"." +
         (ms < 100 ? L"0" : L"") + (ms < 10 ? L"0" : L"") + to_wstring(ms);
}
