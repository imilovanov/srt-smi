 ...
 * @license MIT
 * https://github.com/imilovanov/srt-smi/master/LICENSE.txt
 */
...
class Utils {
 public:
  /**
   * Opens a file into a stream and formats the stream based on the file's
   * encoding.
   *
  ...
  /**
   * Determines if a path to a file system object such as a file or directory is
   * valid.
   *
   * @param path The path string.
   ...
   *     (removed continuously until the string no longer ends with this
   * character).
   ...
   */
  static std::wstring& wstr_replace(std::wstring& subject, std::wstring search,
                                    std::wstring replace);
